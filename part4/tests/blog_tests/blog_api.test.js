...
beforeEach(async () => {
  ...
  let blogObject = new Blog(api_test_helper.initialBlogs[0])
  await blogObject.save()

  blogObject = new Blog(api_test_helper.initialBlogs[1])
  await blogObject.save()
})

describe('Blog Tests related to exercises 4.8-4.14', () => {
  ...
  test('Check that id is used instead of _id in the db as a unique identifier (Exercise 4.9)', async () => {
    const blogs = await api_test_helper.getBlogs()
    blogs.forEach(blog => {
  ...
  test('Test that a valid blog can be added (Exercise 4.10)', async () => {
    ...
    const response = await api_test_helper.getBlogs()
    const contents = response.map(r => r.title)
    expect(response).toHaveLength(api_test_helper.initialBlogs.length + 1)
  ...
  test('Delete a blog (Exercise 4.13)', async () => {
    ...
    const blogsAfterDeletion = await api_test_helper.getBlogs()
    expect(api_test_helper.initialBlogs.length - 1)
